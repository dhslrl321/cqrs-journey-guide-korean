...
일반적으로 이벤트 핸들러 메서드는 프로세스 관리자 인스턴스를 로드하고 이벤트를 프로세스 관리자에게 전달한 다음 pro- cess 관리자 인스턴스를 지속합니다. 이 경우, IRepository 인스턴스는 프로세스 관리자 인스턴스를 지속하고 프로세스 관리자 인스턴스에서 명령 버스로 명령을 보내기 위해 다시 후원할 수 있습니다.
